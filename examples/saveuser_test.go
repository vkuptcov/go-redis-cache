...
func (st *SaveUserSuite) verifyUserPresenceInCache(users ...*User) {
	st.T().Helper()
	var dst []*User
	var keys []string
	for _, u := range users {
...
func (st *SaveUserSuite) verifyUserAbsenceInCache(user *User) {
	st.T().Helper()
	var dst *User
...
func (st *SaveUserSuite) verifyUserByDepartmentPresenceInCache(users ...*User) {
	st.T().Helper()
	for _, u := range users {
		var dst *User
		st.Require().NoError(
